  ...
  >
    ...
    <template v-slot:item.actions="{item}">
      <span :key="item.renderKey" >
        <script-action v-for="action in action_keys" :p-script="item" :action="action" :key="action" ></script-action> &nbsp;
     </span>
    </template>
...
import ScriptAction from './script-action.vue';
...
export default {
    ...
    computed: {
       ...
       refreshTableKey2: {
         get: function() {
               return "actions" + this.refreshTableKey
         }
       }
    },
    mounted () {
