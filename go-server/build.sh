...
env go build -ldflags="-extldflags=-static -X server.minversion=`date -u +.%Y%m%d.%H%M%S`" -o cmdline cmdline.go 
...
env go build -ldflags="-extldflags=-static -X server.minversion=`date -u +.%Y%m%d.%H%M%S`" -o script-manager main.go
